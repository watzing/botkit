...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = function(message, cb) {
            ...
            let fbms = `[${Math.random().toString(36).substring(7)}] fb_msg_send response time`;
            console.time(fbms)
        ...
        bot.getMessageUser = function(message, cb) {
            let fbgu = `[${Math.random().toString(36).substring(7)}] fb_get_usr response time`;
            console.time(fbgu)
            return new Promise(function(resolve, reject) {
                facebook_botkit.api.user_profile(message.user).then(function(identity) {

                    console.timeEnd(fbgu)
